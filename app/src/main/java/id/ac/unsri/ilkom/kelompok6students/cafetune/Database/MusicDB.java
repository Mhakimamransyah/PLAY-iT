...
public class MusicDB extends SQLiteOpenHelper {
    ...
    private static String COL8 =  "Clickable";
    ...
    public void onCreate(SQLiteDatabase db) {
        db.execSQL("CREATE TABLE "+TABLE_NAME+" ("+COL1+" INTEGER PRIMARY KEY,"+COL2+" TEXT,"+COL3+" INTEGER,"+COL4+" TEXT,"+COL5+" TEXT,"+COL6+
                " TEXT,"+COL7+" INTEGER, " +COL8+ " INTEGER)");
    }
    ...
    public void changeStateToUnclickable(int id){
        SQLiteDatabase db = this.getWritableDatabase();
        String sql = "UPDATE "+TABLE_NAME+" SET "+COL8+" = 0 WHERE "+COL1+" = "+id;
        db.execSQL(sql);
        Log.d("unclickable", ": " +id);
    }
    ...
    public int isClickable(int id) {
         SQLiteDatabase db = this.getWritableDatabase();
        String sql = "SELECT " +COL8+ " FROM "+TABLE_NAME+" WHERE "+COL1+" = "+id;
        Cursor data = db.rawQuery(sql,null);

        data.moveToFirst();
        Log.d("cusor", ": " +data.getString(0));

        int res = data.getInt(0);
        data.close();
        return res;
    }
